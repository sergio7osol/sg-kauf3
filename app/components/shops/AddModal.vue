...
<template>
  <UModal v-model:open="open" title="New shop" description="Create a new shop entry">
    <UButton
      label="New shop"
      icon="i-lucide-plus"
      color="primary"
    ...
    <template #body>
      ...
      >
        ...
        <UFormField label="Slug" name="slug" help="Leave empty to auto-generate">
          <UInput v-model="state.slug" placeholder="Slug" />
        </UFormField>

        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <UFormField label="Type" name="type" required>
            <USelect v-model="state.type" :items="shopTypes" />
          </UFormField>
