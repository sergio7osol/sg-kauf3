...
async function onSubmit(event: FormSubmitEvent<PaymentMethodSchema>) {
  ...
  try {
    ...
    const errorMessage = err?.response?.data?.errors?.name?.[0] 
...
<template>
  <div>
    ...
    <div v-if="isLoading" class="py-12 text-center">
      <UProgress animation="carousel" size="lg" color="primary" class="w-48 mx-auto mb-4" />
      <p class="text-sm text-muted">Loading payment methods...</p>
    </div>
    ...
    <template v-else>
      ...
      <UPageCard variant="subtle" class="mb-6">
        <template #header>
          <div class="flex items-center gap-2">
            ...
            <h3 class="font-semibold">Active Payment Methods</h3>
            <UBadge v-if="activePaymentMethods.length" color="primary" variant="subtle" size="sm">
              {{ activePaymentMethods.length }}
            </UBadge>
          </div>
        </template>

        <div v-if="activePaymentMethods.length === 0" class="py-8 text-center text-muted">
          <UIcon name="i-lucide-credit-card" class="w-12 h-12 mx-auto mb-2 opacity-40" />
          <p>No active payment methods</p>
          <p class="text-sm mt-1">Add a payment method to track how you pay for purchases.</p>
        </div>
        ...
        <div v-else class="divide-y divide-default">
          ...
          >
            <div class="flex-1 min-w-0">
              ...
              <p v-if="method.notes" class="text-sm text-muted truncate mt-0.5">
                {{ method.notes }}
              </p>
            </div>
      ...
      <UPageCard v-if="archivedPaymentMethods.length > 0" variant="subtle">
        <template #header>
          <div class="flex items-center gap-2">
            <UIcon name="i-lucide-archive" class="w-5 h-5 text-muted" />
            <h3 class="font-semibold text-muted">Archived</h3>
        ...
        <div class="divide-y divide-default">
          ...
          >
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
                <span class="font-medium truncate">{{ method.name }}</span>
                <UBadge color="neutral" variant="outline" size="xs">Archived</UBadge>
              </div>
              <p v-if="method.notes" class="text-sm text-muted truncate mt-0.5">
                {{ method.notes }}
              </p>
        ...
        <UCard>
          <template #header>
            <div class="flex items-center gap-2 text-error">
              ...
              <h3 class="text-lg font-semibold">Delete Payment Method</h3>
            </div>
          </template>
